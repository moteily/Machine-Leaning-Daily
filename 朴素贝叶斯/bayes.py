...
def testing_naive_bayes():
    ...
    # 4. 训练数据
    p0v, p1v, p_abusive = train_naive_bayes(
        np.array(train_mat), np.array(list_classes))
    # 5. 测试数据
    test_one = ['love', 'my', 'dalmation']
    test_one_doc = np.array(set_of_words2vec(vocab_list, test_one))
    print('the result is: {}'.format(
        classify_naive_bayes(test_one_doc, p0v, p1v, p_abusive)))
...
def spam_test():
    ...
    for i in range(1, 26):
        ...
        try:
            words = text_parse(
                open('data/4.NaiveBayes/email/spam/{}.txt'.format(i)).read())
        except:
            words = text_parse(open(
                'data/4.NaiveBayes/email/spam/{}.txt'.format(i), encoding='Windows 1252').read())
        ...
        try:
            # 添加非垃圾邮件
            words = text_parse(
                open('data/4.NaiveBayes/email/ham/{}.txt'.format(i)).read())
        except:
            words = text_parse(open(
                'data/4.NaiveBayes/email/ham/{}.txt'.format(i), encoding='Windows 1252').read())
        doc_list.append(words)
        full_text.extend(words)
        class_list.append(0)
    # 创建词汇表
