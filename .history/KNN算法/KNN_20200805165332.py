...
def classify0(inX, dataSet, labels, k):
    ...
    for i in range(k):
        ...
        classCount[voteIlabel] = classCount.get(voteIlabel, 0) + 1  #字典中有则返回其值，否则返回default 0
    # 3. 排序并返回出现最多的那个类型
    ...
    maxClassCount = amax(classCount, key=classCount.get)
...
def datingClassTest():
    ...
    datingDataMat, datingLabels = file2matrix(
        'C:/Users/motei/Desktop/ML/KNN算法/2.KNN/datingTestSet2.txt')  # load data setfrom file
    # 归一化数据
...
def handwritingClassTest():
    ...
    trainingFileList = listdir(
        'data/2.KNN/trainingDigits')  # load the training set
    ...
    for i in range(m):
        ...
        trainingMat[i, :] = img2vector(
            'data/2.KNN/trainingDigits/%s' % fileNameStr)
