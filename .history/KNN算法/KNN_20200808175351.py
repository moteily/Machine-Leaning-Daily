...
def classify0(inX, dataSet, labels, k):
    ...
    # 1. 距离计算
    dataSetSize = dataSet.shape[0]  # shape 函数返回矩阵的规模。shape[0]为矩阵行数
    ...
    for i in range(k):
        ...
        classCount[voteIlabel] = classCount.get(
            voteIlabel, 0) + 1  # 字典中有则返回其值，否则返回default 0
    # 3. 排序并返回出现最多的那个类型
...
def file2matrix(filename):
    ...
    for line in fr.readlines():
        ...
        classLabelVector.append(int(listFromLine[-1]))  # text中倒数第一列的数据加进来
        index += 1
...
def autoNorm(dataSet):
    ...
    """
    # 计算每种属性的最大值、最小值、范围
    minVals = dataSet.min(0)    #axis = 0:返回每列最小，1返回每行最小，空：返回整个最小
...
def datingClassTest():
    ...
    datingDataMat, datingLabels = file2matrix(
        'C:/Users/1/Desktop/Machine-Learning-Daily/KNN算法/2.KNN/datingTestSet2.txt')  # load data setfrom file
    # 归一化数据
...
=======
    ax = fig.add_subplot(111)  #总共一个图，第一行第一列
    ax.scatter(datingDataMat[:, 0], datingDataMat[:, 1], 15.0*array(datingLabels), 15.0*array(datingLabels))
>>>>>>> 3e1ea92dd870a8ad5f868df5def5cf74cb0b7425
    plt.show()
...
def handwritingClassTest():
    ...
    trainingFileList = listdir(
        'C:/Users/motei/Desktop/ML/KNN算法/2.KNN/trainingDigits')  # load the training set
    m = len(trainingFileList)
    trainingMat = zeros((m, 1024))  #将二维数据转为一维数据，便于距离差的计算
    ...
    for i in range(m):
        ...
        trainingMat[i, :] = img2vector(
            'C:/Users/motei/Desktop/ML/KNN算法/2.KNN/trainingDigits/%s' % fileNameStr)

    # 2. 导入测试数据
    # iterate through the test set
    testFileList = listdir('C:/Users/motei/Desktop/ML/KNN算法/2.KNN/trainingDigits')
    errorCount = 0.0
    ...
    for i in range(mTest):
        ...
        fileStr = fileNameStr.split('.')[0]  # take off .txt
        classNumStr = int(fileStr.split('_')[0])
        vectorUnderTest = img2vector('C:/Users/motei/Desktop/ML/KNN算法/2.KNN/trainingDigits/%s' % fileNameStr)
